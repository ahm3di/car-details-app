...
class Services{
  ...
  final String searchUrl = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?radius=5000&type=car_repair&key=AIzaSyAJkeLCe0QUS2I1ZDSayaFCFLm8rcwacSA&location=";
  ...
  Future<List<Service>> getNearbyServices() async {
    ...
    data.forEach((f) => services.add(new Service(f["name"],f["rating"].toString(),f["vicinity"],f["place_id"])));
    return services;
  }

  final String detailUrl="https://maps.googleapis.com/maps/api/place/details/json?key=AIzaSyAJkeLCe0QUS2I1ZDSayaFCFLm8rcwacSA&place_id=";

  Future getService(Service service) async{

    var response = await http.get(detailUrl+service.id,headers:{"Accept":"application/json"});
    var result = json.decode(response.body)["result"];

    var weekdays = new List<String>.from(result["opening_hours"]["weekday_text"]);
